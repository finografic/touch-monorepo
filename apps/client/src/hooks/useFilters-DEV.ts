...
export const useFilters = (initialFilters?: OrderFilters) => {
  ...
  // Client-side filtering - apply all filters including current step
  const dataFiltered = useMemo(() => {
    ...
    // Get filters up to and including the current step
    const appliedFilters = Object.entries(filters).filter(([key]) => {
      const filterIndex = FILTER_ORDER.indexOf(key as OrderFieldKey);
      return filterIndex !== -1 && filterIndex <= currentStepIndex;
    });

    log('__DEV: applicableFilters', 'cyan', applicableFilters);
    log('__DEV: appliedFilters', 'lime', appliedFilters);
