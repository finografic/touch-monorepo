...
import { BUTTON_ACTION_VALUES, type PadActionProps, type PadActionType } from 'types/button.types';
...
export const useButtonConfig = (): UseButtonConfigReturn => {
  ...
  const executeAction = useCallback(
    (actionType: string) => {
      switch (actionType) {
        case BUTTON_ACTION_VALUES.NAVIGATE_BACK:
          return handleNavigateBack();
        case BUTTON_ACTION_VALUES.NAVIGATE_NEXT:
          return handleNavigateNext();
        case BUTTON_ACTION_VALUES.CLEAR_COMPLETED:
          return handleClearCompleted();
        case BUTTON_ACTION_VALUES.CANCEL_COMPLETED:
          return handleCancelCompleted();
        case BUTTON_ACTION_VALUES.SELECT_ALL:
          return handleSelectAll();
        case BUTTON_ACTION_VALUES.START_PROCESS:
          // Use route-specific handler
          return getStartHandler()();
        case BUTTON_ACTION_VALUES.FINISH_PRODUCT_PROCESS:
          return handleFinishProductProcess();
        case BUTTON_ACTION_VALUES.PROGRAM_TIME:
          return handleProgramTime();
        case BUTTON_ACTION_VALUES.PROGRAM_PRODUCT:
          return handleProgramProduct();
        case BUTTON_ACTION_VALUES.REPEAT_SELECTION:
          return handleRepeatSelection();
        case BUTTON_ACTION_VALUES.CANCEL_TIME_SESSION:
          return handleCancelTimeSession();
        case BUTTON_ACTION_VALUES.CANCEL_PRODUCT_SESSION:
