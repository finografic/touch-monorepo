...
import { m } from 'i18n/messages/messages';

// Admin Dashboard variants
// export const getAdminDashboard = (inputs: { role: 'admin' | 'public' }) => ({
//   title: m.admin_dashboard_title(inputs),
//   description: m.admin_dashboard_description(inputs),
// });

export const getAdminDashboard = (inputs: { role: 'admin' | 'public' }) => ({
  title: m.admin_dashboard({ role: inputs.role, element: 'title' }),
  description: m.admin_dashboard({ role: inputs.role, element: 'description' }),
});

// Future: Admin Translations variants (when you implement them)
export const getAdminTranslations = (inputs: { role: 'admin' | 'public' }) => ({
...
export const getAdminLanguages = (inputs: { role: 'admin' | 'public' }) => ({
...
export const getAdminSounds = (inputs: { role: 'admin' | 'public' }) => ({
  title: m.admin_sounds_title(),
  description: m.admin_sounds_description(),
});

// Future: Admin Mode variants
export const getAdminMode = (inputs: { role: 'admin' | 'public' }) => ({
...
export const getAdminMaintenance = (inputs: { role: 'admin' | 'public' }) => ({
