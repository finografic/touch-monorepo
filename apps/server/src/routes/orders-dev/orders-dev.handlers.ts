...
import { db } from 'db';
import { orders_dev } from 'db/schemas/orders_dev.schema';
...
export const list: AppRouteHandler<ListRoute> = async (context) => {
  const drinkOrders = await db.query.orders_dev.findMany({
...
export const getOne: AppRouteHandler<GetOneRoute> = async (context) => {
  ...
  const drinkOrder = await db.query.orders_dev.findFirst({
...
export const create: AppRouteHandler<CreateRoute> = async (context) => {
  ...
  const [inserted] = await db.insert(orders_dev).values(drinkOrder).returning();
...
export const patch: AppRouteHandler<PatchRoute> = async (context) => {
  ...
  const [drinkOrder] = await db.update(orders_dev).set(updates).where(eq(orders_dev.id, id)).returning();
...
export const remove: AppRouteHandler<RemoveRoute> = async (context) => {
  const { id } = context.req.valid('param');
  const result = await db.delete(orders_dev).where(eq(orders_dev.id, id));
