...
export const list: AppRouteHandler<ListRoute> = async (context) => {
  ...
  const temperatureProfiles = await db.query.temperature_profiles.findMany({
    ...
    where: temperatures?.length
      ? (fields, operators) =>
          operators.inArray(fields.temperature, temperatures)
      : undefined,
  });
...
export const getByTemperature: AppRouteHandler<GetByTemperatureRoute> = async (
  context
) => {
