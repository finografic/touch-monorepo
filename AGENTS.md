...
- Admin idle overlay: `AdminScreensaver` is mounted from `AdminLayout`; default idle duration is **`ADMIN_SCREENSAVER_INACTIVITY_MS`** in **`apps/client/src/config/app/app.config.ts`** (override via the component `inactivityMs` prop).
