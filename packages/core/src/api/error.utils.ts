...
import { isObject, propIs } from 'guards';
...
export function isErrorResponse(data: unknown): data is ErrorResponse {
  ...
  if (!propIs(data, 'error', isObject)) return false;

  const err = data.error;

  return (
    propIs(err, 'code', (v): v is string => typeof v === 'string') &&
    propIs(err, 'message', (v): v is string => typeof v === 'string')
  );
}
