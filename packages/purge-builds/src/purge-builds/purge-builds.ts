...
import type { PurgeOptions, DeleteProgress } from './purge-builds.types';
import { GLOB_DELETE_EXCLUDE, GLOB_DELETE_INCLUDE } from './purge-builds.config';
...
export async function purge({ dryRun = false, verbose = false, recursive = false }: PurgeOptions = {}) {
  // SAFETY CHECK: Ensure dry-run mode is clearly indicated
  if (dryRun) {
    console.log(chalk.green('🔒 DRY RUN MODE - NO FILES WILL BE DELETED\n'));
    console.log(
      chalk.yellow('⚠️  This is a simulation only. Use without --dry-run to actually delete files.\n'),
    );
  }
  ...
  console.log(chalk.gray('  Mode:', dryRun ? 'DRY RUN (simulation)' : 'LIVE (actual deletion)'));
  ...
  try {
    ...
    for (const pattern of GLOB_DELETE_INCLUDE) {
      // For recursive mode: use pattern as-is
      // For non-recursive mode: remove ** prefix if present
      ...
      if (verbose) {
        ...
        console.log(chalk[dryRun ? 'gray' : 'magenta'](`Working directory: ${baseDir}`));
      }

      const deletedPaths = await deleteAsync(finalPattern, {
        dryRun,
        dot: true,
        cwd: baseDir, // Use cwd instead of joining paths
        onProgress: verbose
          ...
          : undefined,
        ignore: GLOB_DELETE_EXCLUDE, // Use patterns as-is since we're using cwd
      });
